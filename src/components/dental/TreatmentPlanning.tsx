...
export function TreatmentPlanning({
  ...
  const attachToothToTreatment = (id: string) => {
    ...
    onTreatmentsChange(treatments.map(t => t.id === id ? {
      ...t,
      tooth: selectedTooth
    } : t));
  };
  ...
  const getToothStatusBadge = (toothNumber: number | null | undefined) => {
    ...
    return <Badge className={`${colors[status]} text-white text-xs`}>
  ...
  const TreatmentGrid = ({
    ...
    category: 'basic' | 'advanced';
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {treatmentList.map(treatment => <Button key={treatment.name} onClick={() => addTreatment(treatment.name, category)} variant="outline" className="p-4 h-auto flex-col gap-2 hover:bg-primary/5 hover:border-primary/30 transition-all">
  ...
  return <Card className="shadow-[var(--shadow-card)]">
      <CardHeader>
        <CardTitle className="text-foreground text-3xl">Treatment Planning</CardTitle>
      </CardHeader>
      
      <CardContent className="space-y-8">
        {/* Basic Treatments */}
        <TreatmentGrid treatments={basicTreatments} title="Basic Treatments" category="basic" />

        {/* Advanced Treatments */}
        <TreatmentGrid treatments={advancedTreatments} title="Advanced Treatments" category="advanced" />

        {/* Treatment List */}
        <div className="space-y-4">
          <h3 className="text-lg font-medium text-foreground">Planned Treatments</h3>
          
          {treatments.length === 0 ? <div className="text-center p-8 bg-muted/50 rounded-lg">
              <p className="text-muted-foreground">
                No treatments planned yet. Click on treatment options above to add them.
              </p>
            </div> : <div className="space-y-3">
              {treatments.map(treatment => <Card key={treatment.id} className="p-4">
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className="font-medium text-foreground">{treatment.name}</span>
                      {treatment.tooth ? getToothStatusBadge(treatment.tooth) : <Badge variant="secondary" className="text-xs">
                          No tooth linked
                        </Badge>}
