...
export function FileUpload({
  ...
  const removeFile = async (category: FileCategory, index: number) => {
    ...
    try {
      ...
      if (patientId && file.dataUrl) {
        ...
        const {
          error: dbError
        } = await supabase.from('patient_files').delete().eq('patient_id', patientId).eq('file_name', file.name).eq('file_category', category);
  ...
  return <Card className="shadow-[var(--shadow-card)]">
      <CardHeader>
        <CardTitle className="text-foreground text-3xl">Document & Image Management</CardTitle>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          return <div key={category} className={`
              ...
              `}>
                ...
                {files[category].length > 0 && <div className="space-y-1">
                    {files[category].map((file, index) => <div key={index} className="flex items-center justify-between text-xs p-2 bg-background rounded border">
        ...
        {/* File Preview Grid */}
        {Object.values(files).some(categoryFiles => categoryFiles.length > 0) && <div className="space-y-4">
            <h3 className="text-lg font-medium text-foreground">Uploaded Files Preview</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {Object.entries(files).flatMap(([category, categoryFiles]) => categoryFiles.map((file, index) => <Card key={`${category}-${index}`} className="overflow-hidden">
                    <div className="aspect-square bg-muted flex items-center justify-center">
                      {file.type.startsWith('image/') && file.dataUrl ? <img src={file.dataUrl} alt={file.name} className="w-full h-full object-cover" /> : <div className="text-center p-2">
